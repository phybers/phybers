...
```python
def cluster(file_in: str, dir_out: str, assign_thr: int = 6, join_thr: int = 6) -> bool
```
...
-	***dir_out***: directory to store all the results generated by the algorithm.
-	***assign_thr***: a maximum distance threshold for the cluster reassignment in mm, default: $6$
-	***join_thr***: a maximum distance threshold for the cluster merge in mm, default: $6$
...
ffclust.cluster (file_in ='fibers_test.bundles', dir_out = 'ffclust_result', assign_thr = 6, join_thr = 6)
```
