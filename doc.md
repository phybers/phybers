...
-	 ***fiber_input***: tractography data file that contains the fibers of the whole brain of a subject. These must be in the same reference system as the used bundle atlas and be in bundles format.
-	 ***idsubj***: subject identification number, used to label the results.
-	 ***atlasdir***: bundle atlas, with bundles in separated files, sampled at 21 equidistant points. The bundle atlases provided are in different folders.
...
-	 ***result_path***: directory name to store all the results generated by the algorithm.
...
fiberseg(fiber_input = 'subject_01.bundles', idsubj = '01', atlasdir = 'atlas/bundles', atlasInformation = 'atlas/atlas_info.txt', 'resultseg')
```
...
```python
def hclust(fiber_input: str, PartDistance_Threshold: float = 40.0,  MaxDistance_Threshold: float = 30.0, var: int = 3600, work_dir: str) -> None
```
> > >### _HClust  Parameters_

The inputs are:
-	***fiber_input***: tractography data file.
...
-	***work_dir***: directory to store all the results generated by the algorithm.
...
```python
def ffclust(infile: str, output_directory: str, thr_assign: int = 6.0, thr_join: int = 6.0) -> bool
```
> > >### _FFClust  Parameters_

The inputs are:
-	***infile***: input tractography dataset.
-	***output_directory***: directory to store all the results generated by the algorithm.
...
ffclust (infile='fibers_test.bundles', output_directory='ffclust_result', thr_assign=6.0, thr_join=6.0)
```
